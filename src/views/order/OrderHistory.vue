...
<!--                display image in left-->
                <img v-if="order.imageUrl" class="w-100 card-img-top embed-responsive-item" v-bind:src="order.imageUrl">
                <img v-else :src="parseImageUrl(order.imageUrl)" class="w-100 card-img-top embed-responsive-item">
            </div>
								...
								if (response.status == 200) {
									...
									this.orders.forEach((order) => {
										this.orderList.push({
											...
											imageUrl: this.parseImageUrl(order.orderItems[0].product.imageUrl),
											// get total items
