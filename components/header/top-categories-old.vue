          ...
          <li v-for="(item, i) in category_data" class="has-dropdown" :key="i">
            ...
            <ul v-if="item.children" class="tp-submenu">
              <li v-for="(child, i) in item.children" :key="i">
                <a class="pointer" @click="handleSubCategory(child)">{{ child.name }}</a>
              </li>
            </ul>
          </li>
        </ul>
...
// handle parent
const handleSubCategory = (value: ISubCategory) => {
	router.push(`/category/${encodeURIComponent(value.slug)}`);
}
