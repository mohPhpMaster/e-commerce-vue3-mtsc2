  ...
  <section class="tp-product-area pb-70">
    <div class="container">
              ...
              <nav>
                <div class="nav nav-tabs justify-content-center tp-product-tab tp-tab-menu p-relative" id="nav-tab">
                  <template v-for="(tab, i) in categories.slice(0, 4)" :key="i">
                    <button v-if="active_tab === tab" ref="activeRef" @click="handleActiveMarker($event, tab)" :class="`nav-link ${active_tab === tab ? 'active' : ''}`" id="nav_active">
                      {{ tab }}
                      <span class="tp-product-tab-tooltip">{{filteredProducts.length}}</span>
                    </button>
                    <button v-else @click="handleActiveMarker($event, tab)" :class="`nav-link ${active_tab === tab ? 'active' : ''}`">
                      {{ tab }}
                      <span class="tp-product-tab-tooltip">{{filteredProducts.length}}</span>
                    </button>
                  </template>
      ...
      <div class="row">
        <div class="col-xl-12">
          <div class="row">
            <div v-for="item in filteredProducts" :key="item.id" class="col-xl-3 col-lg-4 col-sm-6">
...
const parentNamesArray = await loadCategories().then(data=>data.map((category) => category.parentName));
...
const filteredProducts = computed(() => {
	if (active_tab.value === "All Product") {
		...
		return product_data.filter((p) => p.parentCategory === active_tab.value?.id);
	}
});
